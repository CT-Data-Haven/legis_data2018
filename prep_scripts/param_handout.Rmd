...
See data and interactive maps for all state Senate and House districts on DataHaven's Dashboard page (https://www.ctdatahaven.org/data-dashboard) or hosted on data.world ()
...
pop_age <- tbls$age %>%
  ...
  calc_shares(value = value, digits = 2) %>%
  ungroup() %>%
...
p_age +
  ...
  scale_y_continuous(expand = expand_scale(add = c(0.1, 0.25))) +
...
housing_income %>%
  ...
  scale_x_discrete(expand = expand_scale(mult = 0.7)) +
...
```{r cws, fig.height=7, fig.width=7.2, eval=is_senate}
cws %>%
  arrange(topic) %>%
  ...
  scale_y_barcontinuous(breaks = NULL) +
  scale_x_discrete(expand = expand_scale(mult = 0.6)) +
  scale_fill_manual(values = c(state = "gray60", district = bupu7[5])) +
